...
chess::Move Search::start_search(Board& board, int depth, int movetime, int wtime, int btime, int winc, int binc) {    
    ...
    // History heuristics removed for debugging
    // for (int i = 0; i < 15; ++i) {
    //     for (int j = 0; j < 64; ++j) {
    //         history_scores[i][j] /= 2;
    //     }
    // }
    ...
    for (int i = 1; i <= 60; ++i) {
        ...
        // --- Aspiration Window Removed ---
        // We now search with a full window every time.
        int64_t alpha = CHECKMATE_EVAL;
        int64_t beta = -CHECKMATE_EVAL;

        // --- 'while(true)' re-search loop removed ---
        ...
        if (!moveList.empty()) {
            ...
            board.make_move(m);
            if (board.is_position_legal()) {
                // Standard PVS: First move is a full window search
                int64_t s = -negamax(board, i - 1, 1, -beta, -current_alpha);
                if (s > current_alpha) {
                    current_alpha = s;
                    best_move_this_iter = m;
        ...
        for (size_t j = 1; j < moveList.size(); ++j) {
            ...
            // Other moves are zero-window (scout) searches
            futures.push_back({pool.enqueue(&Search::negamax, this, b_copy, i - 1, 1, -current_alpha - 1, -current_alpha), moveList[j]});
        }
        ...
        for (auto& [future, move] : futures) {
            ...
            // If the zero-window search failed high, we must re-search with a full window
            if (s > current_alpha && s < beta) {
                Board b_copy = board;
                b_copy.make_move(move);
                // This re-search is not parallelized in this simple version,
                // but it's crucial for correctness.
                s = -negamax(b_copy, i - 1, 1, -beta, -s);
            }
