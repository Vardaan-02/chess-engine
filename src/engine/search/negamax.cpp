...
int64_t Search::negamax(Board& board, int depth, int ply, int64_t alpha, int64_t beta)
{
    ...
    // --- Pruning Removed For Debugging ---
    // Null Move Pruning (NMP) block removed.
    // ---
    ...
    while(!(move = orderer.get_next_move()).is_null()){
        ...
        // --- Reductions Removed For Debugging ---
        // Late Move Reductions (LMR) logic removed.
        // This is now a plain negamax call.
        int64_t score = -negamax(board, depth - 1, ply+1, -beta, -alpha);
        
        board.unmake_move(move);

        if (score >= beta) {
            // Killer/History updates removed for debugging
            entry = { board.zobrist_key, (uint8_t)depth, score, TTEntry::LOWER_BOUND, move };
            TT.store(entry);
            return beta; 
        }
    ...
    if (legal_moves_found == 0) {
        int64_t score = board.checks ? CHECKMATE_EVAL + ply : DRAW_EVAL;
        entry = { board.zobrist_key, (int8_t)MAX_PLY, score, TTEntry::EXACT, {} };
        TT.store(entry);
        return score;
    }
